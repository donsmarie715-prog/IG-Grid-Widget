...
export default async function handler(req, res) {
  try {
    ...
    if (platform) filters.push({ property: "Platform", multi_select: { contains: platform } });

    const query = { database_id: databaseId, sorts: [{ property: "Post Date", direction: "ascending" }] };
    ...
    const items = resp.results.map(page => {
      ...
      const title = p["Post Title"]?.title?.[0]?.plain_text || p["Name"]?.title?.[0]?.plain_text || "Untitled";
